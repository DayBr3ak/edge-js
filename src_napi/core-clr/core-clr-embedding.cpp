...
int CoreClrInit() {
  ...
  if (!pal::getcwd(&currentDirectory)) {
    THROW_EXCEPTION("Unable to get the current directory.");
    return R_FAIL;
  }
  ...
  char tempEdgeNodePath[PATH_MAX];
    ...
    GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, (LPCTSTR)CoreClrInit, &moduleHandle);
  ...
  pal::string_t edgeApp;
  pal::clr_palstring("C:\\Users\\bgroeneveld.SIFAM\\home\\edge-js\\build\\Debug", &edgeApp);
